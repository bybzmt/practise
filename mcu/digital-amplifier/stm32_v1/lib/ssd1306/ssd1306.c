...
uint8_t ssd1306_WriteString(char* str, FontDef *Font, SSD1306_COLOR color) {
    ...
    while (*str) {
        if (*str < Font->AsciiOffset || *str > Font->AsciiOffset + Font->Num)
            return len;

        const uint16_t *data = Font->data[*str - Font->AsciiOffset];

        if (ssd1306_WriteChar(data, Font->Width, Font->Height, color) == false) {
            return len;
        }
...
void ssd1306_FillRectangle(uint8_t x, uint8_t y, uint8_t width, uint8_t height, SSD1306_COLOR color)
{
    for (uint8_t w=0; w<width; w++) {
        for (uint8_t h=0; h<height; h++) {
            ssd1306_DrawPixel(x+w, y+h, color);
        }
    }
}
