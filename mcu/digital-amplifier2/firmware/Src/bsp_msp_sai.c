...
static void MY_SAI_OUT_MspInit(SAI_HandleTypeDef *hsai)
{
    audio_sai_clockConfig(hsai->Init.AudioFrequency);
    ...
    __HAL_LINKDMA(hsai, hdmatx, audio.hdma_tx);

    /* Deinitialize the Stream for new transfer */
    HAL_DMA_DeInit(&audio.hdma_tx);

    /* Configure the DMA Stream */
    HAL_DMA_Init(&audio.hdma_tx);
...
{
    HAL_DMA_IRQHandler(&audio.hdma_tx);
}
