...
int main(int argc, char *argv[])
{
	...
	cairo_surface_t* surface = cairo_svg_surface_create("taichi.svg", SIZE + 1, SIZE + 1);
	...
	/* 绘制阴阳线 */
    cairo_set_source_rgba(cr, 0, 0, 0, 1);
    cairo_arc(cr, CX -1, CY - R / 2, R / 2, ANGLE(-90), ANGLE(90));
    cairo_fill(cr);
    cairo_set_source_rgba(cr, 1, 1, 1, 1);
    cairo_arc(cr, CX+1, CY + R / 2, R / 2, ANGLE(90), ANGLE(270));
    cairo_fill(cr);
